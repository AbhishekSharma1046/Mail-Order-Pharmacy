...
namespace AuthorizationMicroService.Providers
{
    public class TokenProvider : ITokenProvider<MemberDetails>
    {
        ...
        private readonly AuContext _Db;
        private readonly List<MemberDetails> members = new List<MemberDetails>()
        {
            new MemberDetails{Id=1, Name="Saurabh", Email="san@gmail.com", Password="san", Location="Haldwani"},
            new MemberDetails{Id=2, Name="Ben", Email="bt123@gmail.com", Password="hello", Location="Haldwani"}
        };

        public TokenProvider(ITokenRepository<MemberDetails> tokenRepository, AuContext Db)
        {
            _Db = Db;
            this.tokenRepository = tokenRepository;
        }

        public TokenProvider(ITokenRepository<MemberDetails> @object)
        {
        }
        ...
        public UserData GetToken(MemberDetails entity)
        {
            ...
            try
            {
                ...
                var res = _Db.TbPrescriptionDetails.SingleOrDefault(x => x.Email.Equals(entity.Email) && x.Password.Equals(entity.Password));
